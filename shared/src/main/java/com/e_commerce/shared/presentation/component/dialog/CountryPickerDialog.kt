...
import com.e_commerce.shared.presentation.component.CircleCheckbox
...
private fun List<Country>.filterByCountry(query: String): List<Country> {
    ...
    return this.filter { country ->
        country.name.contains(
            ...
            other = query,
            ignoreCase = true
        ) || (numQuery != null && country.dialCode == numQuery)
    }
}
...
private fun CountryPickerDialogPrev() {
    PreviewTheme {
        CountryPickerDialog(
            country = Country.entries.shuffled().first(),
            onDismiss = {},
            onConfirmClick = {}
        )
    }
